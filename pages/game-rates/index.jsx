...
const GameRates = ({ data }) => {
  ...
  const renderGameRates = () => {
    ...
    if (!obj) {
      return <div className="text-white">no record found!</div>;
    }
    ...
    return data.map((game, index) => {
      ...
      let val = game[key];
      return (
        <div key={index} className="row game-list-inner5 mt-3 p-2">
  ...
  return (
    <div className="maxWidth">
      <div  style={{ textAlign: "center" }}>
        ...
        <div className="p-3" style={{height: "100vh"}}>{renderGameRates()}</div>
      </div>
    </div>
  );
};
