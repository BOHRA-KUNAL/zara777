  ...
  const renderWinningRecords = () => {
    return winningRecords.map((item, index) => (
      <div key={index} style={{
        ...
        <div style={{ fontWeight: 'bold', fontSize: '18px', marginBottom: '10px', color: '#3d0657', textAlign: 'center' }}>{item.game_name}</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', fontSize: '14px' }}>
          {!gali && <div><strong style={{color: '#3d0657'}}>Session:</strong> {item.session}</div>}
          {!gali && <div><strong style={{color: '#3d0657'}}>Open Digit:</strong> {item.digits}</div>}
          <div><strong style={{color: '#3d0657'}}>{gali ? "Digit" : "Close Digit"}:</strong> {item.closedigits}</div>
          <div><strong style={{color: '#3d0657'}}>Amount:</strong> {item.amount}</div>
          <div><strong style={{color: '#3d0657'}} >Game Type:</strong> {item.pana}</div>
          <div><strong style={{color: '#3d0657'}}>Date and Time:</strong> {item.wining_date}</div>
        </div>
      </div>
    ));
  };
